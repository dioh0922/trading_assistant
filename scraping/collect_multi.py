    ...
    for period in ("10y", "5y"):
      try:
        stock_data = yf.download(
          tickers=yf_ticker, period=period, interval="1d", progress=False
        )
  ...
  if os.path.exists(output_csv):
    with lock:
      ...
      if completed_count % 100 == 0:
        print(
          f"進捗: {completed_count}/{total} (成功: {success_count}, 失敗: {failed_count}, スキップ: {skipped_count})"
        )
    return
...
def main():
  ...
  print(
    f"完了 - 成功：{success_count}件, 失敗：{failed_count}件, スキップ：{skipped_count}件"
  )
