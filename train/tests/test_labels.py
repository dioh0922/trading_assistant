...
def test_triple_barrier_flat() -> None:
    ...
    labels = result["label"].to_list()
    assert all(l == "timeout" for l in labels)
...
def test_build_labels_task1(tmp_path: Path) -> None:
    ...
    result = build_labels_task1(tmp_path / "prices.parquet", output, horizon_days=5, up_threshold=0.01)
...
def test_build_labels_task2(tmp_path: Path) -> None:
    ...
    result = build_labels_task2(tmp_path / "prices.parquet", output, upper=0.10, lower=-0.05, max_days=60)
    assert output.exists()
