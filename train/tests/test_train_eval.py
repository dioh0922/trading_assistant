...
def test_train_pipeline(temp_workspace):
  ...
  n_rows = 100
  dates = [
    datetime.date(2026, 1, 1) + datetime.timedelta(days=i) for i in range(n_rows)
  ]
  ...
  df = pl.DataFrame(
    {
      ...
      "label": [
        "timeout" if i % 3 == 0 else ("upper" if i % 3 == 1 else "lower")
        for i in range(n_rows)
      ],
      "fold": [i % 3 for i in range(n_rows)],  # 3 folds
      "split_type": [
        ["train", "valid", "time_holdout", "stock_holdout"][i % 4]
        for i in range(n_rows)
      ],
...
"""
  ...
  try:
    ...
    predict_exit(
      temp_workspace / "config" / "config.yaml", "TICKER_A", entry_price=100.0
    )
  finally:
