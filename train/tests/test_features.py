...
def test_feature_columns_exist() -> None:
    ...
    expected_cols = [
        ...
        "atr_14", "rsi_14", "volume_ratio_20d", "range_position_60d",
    ]
...
def test_no_future_leakage() -> None:
    ...
    for w in [5, 20, 60]:
        for t_idx in range(w + 1, 100):
            ...
            assert abs(return_val - expected) < 1e-10, (
...
def _make_multi_ticker_prices(n_days: int = 30, tickers: list[str] | None = None) -> pl.DataFrame:
    if tickers is None:
        tickers = ["A", "B", "C"]
    np.random.seed(42)
    base = date(2024, 1, 1)
