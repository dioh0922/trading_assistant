...
def build_train_eval_data(
  ...
  if task in ("task1", "task2"):
    ...
    dataset_df = dataset_df.with_columns(
      pl.when(pl.col("label") == "upper").then(1).otherwise(0).alias("target")
    )
...
def run_training(
  ...
  for k in range(n_folds):
    ...
    if is_directory_split:
      ...
      _, train_df, _ = build_train_eval_data(
        train_df, task, label_encoder=label_encoder
      )
      if len(train_df) == 0:
    ...
    if len(valid_df) > 0:
      _, valid_df, _ = build_train_eval_data(
        valid_df, task, label_encoder=label_encoder
      )
      X_val = valid_df.select(feature_cols).to_pandas()
      y_val = valid_df["target"].to_numpy()
      callbacks.append(
        early_stopping(stopping_rounds=lgb_config.early_stopping_rounds, verbose=False)
      )
    ...
    model.fit(X_train, y_train, eval_set=eval_set, callbacks=callbacks, **fit_params)
