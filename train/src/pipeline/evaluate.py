...
def generate_report_multiclass(
    ...
    # Detailed report
    report_time = classification_report(y_true_time, y_pred_time, target_names=classes, zero_division=0)
    report_stock = classification_report(y_true_stock, y_pred_stock, target_names=classes, zero_division=0)
...
def generate_report_binary(
    ...
    report_time = classification_report(y_true_time, y_pred_time, target_names=["timeout", "upper"], zero_division=0)
    report_stock = classification_report(y_true_stock, y_pred_stock, target_names=["timeout", "upper"], zero_division=0)
...
def run_evaluation(
    ...
    if task in ("task1", "task2"):
        ...
        generate_report_multiclass(task, y_time, pred_time, y_stock, pred_stock, label_classes, report_path)
        ...
        generate_report_binary(task, y_time, avg_proba_time, y_stock, avg_proba_stock, report_path)
        ...
        generate_report_regression(task, y_time, avg_pred_time, y_stock, avg_pred_stock, report_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Phase 7: Evaluation")
    parser.add_argument("--task", choices=["task1", "task2", "task3_hit", "task3_days"], required=True, help="Task to evaluate")
