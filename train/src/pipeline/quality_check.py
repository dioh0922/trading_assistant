...
def flag_quality_issues(prices: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    if flags:
        ...
        flag_df = pl.DataFrame(schema={"ticker": pl.Utf8, "date": pl.Date, "reason": pl.Utf8})

    log.info("quality_check: %d flagged rows", len(flag_df))

    flagged_dates = flag_df.select(
        pl.col("ticker"), pl.col("date"), pl.lit(True).alias("is_flagged"),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-clean", type=Path, default=Path("data/processed/prices_clean.parquet"))
    parser.add_argument("--output-flags", type=Path, default=Path("reports/quality_check_flags.csv"))
