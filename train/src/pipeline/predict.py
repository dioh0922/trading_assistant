...
def predict_exit(
  ...
  print(f"銘柄: {ticker} (最新日付: {latest_date}, 終値: {latest_close:.1f}円)")
  print(
    f"基準価格: {ref_price:.1f}円 (利確価格 (+{barrier['upper_barrier'] * 100:.1f}%): {upper_price:.1f}円, 損切り価格 ({barrier['lower_barrier'] * 100:.1f}%): {lower_price:.1f}円)"
  )
  print("-" * 50)
...
def main() -> None:
  ...
  trend_parser.add_argument(
    "--ticker", type=str, required=True, help="Stock ticker code"
  )
  trend_parser.add_argument(
    "--data-dir", type=Path, default=None, help="Directory containing feature data"
  )
  ...
  exit_parser.add_argument(
    "--entry-price", type=float, default=None, help="Position entry price"
  )
  exit_parser.add_argument(
    "--data-dir", type=Path, default=None, help="Directory containing feature data"
  )
  ...
  target_parser = subparsers.add_parser(
    "target", help="Predict target return hit (Task 3)"
  )
  target_parser.add_argument(
    "--ticker", type=str, required=True, help="Stock ticker code"
  )
  target_parser.add_argument(
    "--target-price", type=float, required=True, help="Target price to hit"
  )
  target_parser.add_argument(
    "--data-dir", type=Path, default=None, help="Directory containing feature data"
  )
