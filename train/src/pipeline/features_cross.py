...
def compute_cross_sectional(
  ...
  ranked = ranked.with_columns(
    [
      (pl.col("daily_return") - pl.col("_market_avg_return")).alias(
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--features-single",
    type=Path,
    default=Path("data/features/features_single.parquet"),
  )
  parser.add_argument(
    "--prices-clean", type=Path, default=Path("data/processed/prices_clean.parquet")
  )
  parser.add_argument(
    "--output", type=Path, default=Path("data/features/features_cross.parquet")
  )
