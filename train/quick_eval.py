...
def build_features_single_ticker(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for w in (5, 20, 60):
        ...
        out[f"volatility_{w}d"] = np.log(out["close"] / out["close"].shift(1)).rolling(w).std()
...
def build_labels(prices: pd.DataFrame, upper: float, lower: float, max_days: int) -> pd.DataFrame:
    parts = []
    for ticker, group in prices.groupby("ticker"):
...
# ──────────────────────────────────────────────────────────────
LABEL_JP = {"upper": "利確ライン到達", "lower": "損切りライン到達", "timeout": "様子見（未到達）"}


def predict_for_ticker(model: LGBMClassifier, metadata: dict, data_dir: Path, ticker: str) -> None:
    ...
    print("-" * 50)
    print(f"→ 最も可能性が高いのは「{LABEL_JP.get(classes[best_idx], classes[best_idx])}」"
...
FEATURE_COLUMNS = [
    ...
    "ma_dev_5", "ma_dev_20", "ma_dev_60",
    "volatility_5d", "volatility_20d", "volatility_60d",
    "rsi_14", "volume_ratio_20d", "range_position_60d",
    "atr_14",
]
...
def run(
    ...
    log.info("building triple-barrier labels (task2: upper=%.2f, lower=%.2f, max_days=%d)...",
              upper, lower, max_days)
    ...
    log.info("dataset rows: %d, label distribution:\n%s", len(dataset), dataset["label"].value_counts())
    ...
    for train_ratio in split_ratios:
        ...
        if train_df.empty or test_df.empty:
            log.warning("train_ratio=%.2f: train/testのどちらかが空。スキップします。", train_ratio)
            continue
        ...
        log.info("training LightGBM classifier (train_ratio=%.2f, class_weight='balanced', sample_weight=yes)...",
                  train_ratio)
        model = LGBMClassifier(
        ...
        print(f"  test期間中の平均20日リターン: {test_avg_ret:+.4f} ({test_avg_ret*100:+.2f}%)")
        ...
        print("クロスセクション特徴量なし）。本番評価は必ずフルパイプラインで行ってください。")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="実装途中のパイプラインの精度を簡易チェックする")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="学習・評価を行い、モデルをファイルに保存する")
    train_parser.add_argument("--data-dir", type=Path, required=True, help="CSVが入ったディレクトリ")
    train_parser.add_argument("--model-dir", type=Path, default=Path("models/task2"),
                               help="モデルの保存先ディレクトリ（デフォルト: models/task2）")
    train_parser.add_argument("--upper", type=float, default=0.10, help="利確ライン（例: 0.10 = +10%）")
    train_parser.add_argument("--lower", type=float, default=-0.05, help="損切りライン（例: -0.05 = -5%）")
    train_parser.add_argument("--max-days", type=int, default=60, help="時間バリア（日数）")
    train_parser.add_argument("--sample-tickers", type=int, default=None,
                               help="使用する銘柄数を制限する場合に指定")
    train_parser.add_argument("--multi-split", action="store_true",
                               help="複数のtrain/test比率(0.6,0.7,0.8)で比較評価する")

    predict_parser = subparsers.add_parser("predict", help="保存済みモデルを読み込み、再学習せずに予測する")
    ...
    predict_parser.add_argument("--data-dir", type=Path, required=True,
                                 help="予測対象銘柄のCSVが入ったディレクトリ")
    predict_parser.add_argument("--ticker", type=str, required=True, help="予測したい銘柄コード")
