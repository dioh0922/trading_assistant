...
def ensemble_predict_proba(models: list, X: pd.DataFrame, label_classes: list[str]) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    if np.issubdtype(np.array(raw_classes).dtype, np.number):
        ...
        log.info("整数エンコードされたラベルを文字列に変換しました: %s -> %s",
                  list(raw_classes), list(classes))
    else:
...
def load_holdout_dataset(
    ...
    log.info("holdout rows: %d%s", len(holdout),
...
# 1. 基本指標
# ──────────────────────────────────────────────────────────────
def report_basic_metrics(y_true: pd.Series, y_pred: np.ndarray, lines: list[str]) -> None:
    ...
    lines.append(f"### ベースライン：多数派クラス予測（ホールドアウト内の多数派 = {majority}）")
    lines.append("```")
    ...
    if low_recall:
        lines.append(f"⚠ 警告: 以下のクラスはrecallが極端に低く(<5%)、実用に耐えません: {low_recall}")
    else:
        lines.append("クラス別recallは全クラスで5%を上回っています（最低限のチェックはクリア）。")
    lines.append("")
...
def report_confidence_sweep(
    ...
    if len(valid) >= 2:
        if valid["accuracy"].is_monotonic_increasing:
            lines.append("→ 閾値を上げるほどaccuracyが単調に改善しています。確信度は信頼できる指標として機能している可能性が高いです。")
        else:
...
def run(
    ...
    holdout = load_holdout_dataset(dataset_path, split_col, split_value, feature_columns, label_col)
    ...
    lines.append(f"- バリア設定: upper={barrier_config['upper']}, lower={barrier_config['lower']}, "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="銘柄軸ホールドアウトに対する基本指標・確信度フィルタ評価（フル実装パイプライン対応版）")
    parser.add_argument("--model-dir", type=Path, required=True, help="fold*.joblib + metadata.jsonがあるディレクトリ")
    parser.add_argument("--dataset-path", type=Path, required=True,
                         help="特徴量・ラベル・split列を持つ結合済みデータセット（parquetまたはcsv）")
    parser.add_argument("--split-col", type=str, default="split_type", help="split種別が入っている列名")
    parser.add_argument("--split-value", type=str, default="stock_holdout",
