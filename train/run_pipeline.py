...
def run_pipeline(
  ...
  if should_skip and features_single_path.exists():
    ...
    log.info(
      "Phase 3a [features_single]: %s -> %s", prices_clean_path, features_single_path
    )
  ...
  for t in tasks:
    ...
    if should_skip and dataset_path.exists():
      ...
      log.info(
        "Phase 5 %s [dataset_split]: labels_%s.parquet -> %s", t, t, dataset_path
      )
      labels_path = labels_dir / f"labels_{t}.parquet"
      features_path = (
        features_cross_path if features_cross_path.exists() else features_single_path
      )
      build_dataset_split(
...
def main() -> None:
  ...
  parser.add_argument(
    "--sample-tickers", type=int, default=None, help="limit to first N tickers"
  )
  parser.add_argument(
    "--skip-existing",
    action="store_true",
  ...
  parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="re-run even with --skip-existing",
  )
  parser.add_argument(
    "--extract-ratio",
    type=float,
