...
def resolve_raw_label(
  classes: np.ndarray, predicted_label: str, label_classes: list[str]
):
  """
...
def run_selftest(n_folds: int = 3, n_samples: int = 3000, seed: int = 0) -> bool:
  ...
  log.info(
    "selftest: target sample feature_a=%.3f, predicted=%s, proba=%s",
    feature_a[target_idx],
    predicted_label,
    dict(zip(classes, proba.round(3))),
  )

  shap_series, elapsed = compute_ensemble_shap(
    models, X_single, predicted_label, list(classes)
  )
  ...
  print(
    f"  - feature_aが他の特徴量の2倍以上の絶対値を持つか: "
...
def compute_single_comparison(models: list, metadata: dict, detail: dict) -> dict:
  """
  ...
  global_importance_map = {
    f["feature"]: f["global_importance"] for f in detail["features"]
  }
  ...
  df["global_rank"] = (
    df["global_importance"].rank(ascending=False, method="min").astype(int)
  )
  df["shap_rank"] = (
    df["shap_value"].abs().rank(ascending=False, method="min").astype(int)
  )
...
def run_analyze(
  model_dir: Path, input_json: Path, top_k: int, report_out: Path | None
) -> None:
  ...
  lines.append("")
  lines.append(
    "| shap_rank | feature | value | shap_value | global_importance | global_rank |"
  )
  lines.append("|---|---|---|---|---|---|")
  ...
  lines.append(
    f"- Jaccard類似度: {jaccard:.2f}（1.0で完全一致、低いほど「モデル全体の重要度」と"
  )
  lines.append("  「この1件への実際の寄与」が異なることを意味する）")
  lines.append("")
  if jaccard < 0.7:
    lines.append(
      "→ global_importanceとSHAPのランキングに無視できない差がある。"
      "global_importanceだけで『なぜこの予測になったか』を説明するのは"
      "妥当ではないことが、このデータでも裏付けられた。"
    )
  else:
    lines.append(
      "→ 今回のサンプルでは両者のランキングが比較的近い。"
  ...
  lines.append(
    "  「モデル全体の重要度の序列と、この1件での効き方の序列が別物」であることを意味する。"
  )
  ...
  lines.append(
    f"## 顕著な順位変動 上位{top_k}件（global_rank と shap_rank の差が大きい順）"
  )
  lines.append("")
...
def run_batch(
  ...
  lines.append("")
  lines.append(
    f"- 対象ファイル数: {len(json_paths)}（成功: {len(records)}, 失敗: {len(errors)}）"
  )
  ...
  for label, g in df.groupby("predicted_label"):
    lines.append(
      f"| {label} | {len(g)} | {g['jaccard'].mean():.3f} | {g['spearman_corr'].mean():.3f} |"
    )
  lines.append("")
  ...
  lines.append("")
  lines.append(
    "| file | ticker | analyzed_at | predicted_label | confidence | jaccard | spearman_corr |"
  )
  lines.append("|---|---|---|---|---|---|---|")
  ...
  if std >= 0.3:
    lines.append(
      f"→ spearman_corrの標準偏差が{std:.3f}と大きく、一致度自体がケースによって大きく"
      f"振れている。global_importanceは参考程度に留め、個別分析では毎回SHAPを"
      f"計算する現行方針を支持する結果。"
    )
  else:
    lines.append(
      f"→ spearman_corrの標準偏差は{std:.3f}とそれほど大きくない。"
  ...
  if plot_out is not None:
    try:
      ...
      log.warning(
        "matplotlibが見つからないため、ヒストグラム画像の出力をスキップしました。"
      )
...
def main() -> None:
  parser = argparse.ArgumentParser(
    description="SHAP値によるper-instance特徴量寄与の実験"
  )
  subparsers = parser.add_subparsers(dest="command", required=True)

  selftest_parser = subparsers.add_parser(
    "selftest", help="合成データでの妥当性確認（4a）"
  )
  ...
  if args.command == "selftest":
    passed = run_selftest(
      n_folds=args.n_folds, n_samples=args.n_samples, seed=args.seed
    )
