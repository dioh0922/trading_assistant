...
def load_all_records(batch_dir: Path) -> pd.DataFrame:
    ...
    dedup_keys = ["ticker", "analyzed_at", "file"] if "file" in combined.columns else ["ticker", "analyzed_at"]
    combined = combined.drop_duplicates(subset=dedup_keys, keep="first").reset_index(drop=True)

    n_after = len(combined)
    if n_after < n_before:
        log.info("重複除去: %d件 -> %d件（%d件の重複を削除）", n_before, n_after, n_before - n_after)
...
def naive_label_comparison(df: pd.DataFrame, lines: list[str]) -> None:
    ...
    groups = {label: g["spearman_corr"].to_numpy() for label, g in df.groupby("predicted_label")}
    counts = {label: len(v) for label, v in groups.items()}
    lines.append(f"- サンプル内訳: {counts}")

    if "upper" in groups and "lower" in groups and len(groups["upper"]) >= 2 and len(groups["lower"]) >= 2:
        stat, p = mannwhitneyu(groups["upper"], groups["lower"], alternative="two-sided")
        lines.append(f"- upper vs lower の spearman_corr（Mann-Whitney U検定）: "
                     f"U={stat:.1f}, p={p:.4f}")
        lines.append(f"  upper平均={groups['upper'].mean():.3f} / lower平均={groups['lower'].mean():.3f}")
        lines.append(f"  {'✅ 有意差あり（p<0.05）' if p < 0.05 else '△ 有意差なし（この検定単体では）'}")
    else:
...
def clustered_label_comparison(df: pd.DataFrame, lines: list[str]) -> None:
    lines.append("### 銘柄クラスタを考慮した検定（銘柄×predicted_labelごとに平均してから検定）")
    lines.append("")
    lines.append("同一銘柄が複数日にわたって同じ結論を出しやすいこと（自己相関）を考慮し、")
    lines.append("まず「銘柄×predicted_label」単位でspearman_corrを平均し、その平均値どうしを比較する。")
    lines.append("")
    ...
    for _, r in ticker_label_means.sort_values(["predicted_label", "ticker"]).iterrows():
        lines.append(f"| {r['ticker']} | {r['predicted_label']} | {r['mean']:.3f} | {int(r['count'])} |")
    lines.append("")
    ...
    if "upper" in groups and "lower" in groups and len(groups["upper"]) >= 2 and len(groups["lower"]) >= 2:
        stat, p = mannwhitneyu(groups["upper"], groups["lower"], alternative="two-sided")
        lines.append(f"- upper vs lower（銘柄単位平均, Mann-Whitney U検定）: U={stat:.1f}, p={p:.4f}")
        lines.append(f"  upper平均={groups['upper'].mean():.3f} / lower平均={groups['lower'].mean():.3f}")
        lines.append(f"  {'✅ 有意差あり（p<0.05）' if p < 0.05 else '△ 有意差なし（銘柄単位に集約すると弱まる/消える可能性あり）'}")
    else:
        lines.append("- 銘柄単位で見ると、upper/lowerいずれかの銘柄数が2件未満のため、検定を省略。"
...
def clustered_confidence_correlation(df: pd.DataFrame, lines: list[str]) -> None:
    ...
    lines.append("")
    lines.append("観測日数が3日以上ある銘柄についてのみ、銘柄内でconfidenceとspearman_corrの")
    lines.append("相関係数を個別に計算し、その分布（銘柄をまたいだ平均）を見る。")
    lines.append("これにより「特定の1銘柄の観測回数が多いせいで、全体の相関が引きずられる」")
    lines.append("ことを避ける。")
    lines.append("")
    ...
    if not per_ticker_corrs:
        lines.append("- 観測日数3日以上の銘柄がないため、銘柄内相関を計算できません。"
                     "（データを蓄積してから再実行してください）")
        lines.append("")
        return
    ...
    lines.append(f"- 銘柄をまたいだ平均相関係数: {mean_corr:+.3f}（対象銘柄数: {len(corr_df)}）")
    if len(corr_df) >= 2:
        # 銘柄ごとの相関係数を「1サンプル」とみなした簡易的な片側検定（符号検定に近い簡便法）
        positive = (corr_df["corr"] > 0).sum()
        negative = (corr_df["corr"] < 0).sum()
        lines.append(f"- 符号の内訳: 正の銘柄 {positive} / 負の銘柄 {negative}"
...
# メイン処理
# ──────────────────────────────────────────────────────────────
def run(batch_dir: Path, report_out: Path | None, combined_csv_out: Path | None) -> None:
    ...
    lines.append("")
    lines.append("**注意**: 以下の「素朴な検定」は同一銘柄の複数日データをすべて独立サンプル")
...
def main() -> None:
    ...
    parser.add_argument("--batch-dir", type=Path, default=Path("batch"),
                         help="experiment_shap.py batch の出力先ディレクトリ（run_*/records.csv を探す）")
    parser.add_argument("--report-out", type=Path, default=None)
    parser.add_argument("--combined-csv-out", type=Path, default=None,
                         help="重複除去後の統合済みrecordsを保存するCSVパス")
    args = parser.parse_args()

    run(batch_dir=args.batch_dir, report_out=args.report_out, combined_csv_out=args.combined_csv_out)
