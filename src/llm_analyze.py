...
DETAIL_FILENAME_RE = re.compile(
  r"^(?P<ticker>.+)_detail_(?P<timestamp>\d{8}_\d{6})\.json$"
)
...
def extract_flagged_tickers(report_path: Path) -> list[str]:
  ...
  if not tickers:
    log.warning(
      "'## 要注意銘柄' セクションは見つかりましたが、銘柄コードを1件も抽出できませんでした。"
    )
  else:
...
def compute_population_reliability(combined_records: pd.DataFrame | None) -> dict:
  ...
  for label in ["upper", "lower", "timeout"]:
    ...
    if len(g) > 0:
      result[label] = {
        "mean_spearman_corr": float(g["spearman_corr"].mean()),
        "n": len(g),
      }
    else:
      result[label] = POPULATION_RELIABILITY.get(
        label, {"mean_spearman_corr": None, "n": 0}
      )
  return result
...
def call_llm_gemini(system_prompt: str, user_content: str, model: str) -> str:
  ...
  if response.candidates:
    ...
    if finish_reason and str(finish_reason) not in ("FinishReason.STOP", "STOP"):
      log.warning(
        "Geminiの応答が完了理由 '%s' により途切れた可能性があります。", finish_reason
      )
...
def analyze_one_ticker(
  ...
  try:
    ...
    reliability = compute_explanation_reliability(
      actual_ticker, predicted_label, combined_records
    )
    log.info(
      "[%s] explanation_reliability: source=%s, mean_spearman_corr=%s, n=%s",
      ticker,
      reliability["source"],
      reliability["mean_spearman_corr"],
      reliability["n"],
    )
...
def run_batch(
  ...
  summary_lines.append(
    f"- 成功: {sum(1 for r in results if r['error'] is None)}"
  ...
  for result in results:
    ...
    if dry_run:
      ...
      summary_lines.append(
        f"- explanation_reliability: {result['reliability']['note']}"
      )
      summary_lines.append("")
    else:
...
def load_combined_records(combined_records_path: Path | None) -> pd.DataFrame | None:
  ...
  if not combined_records_path.exists():
    log.warning(
      "%s が見つからないため、母集団のフォールバック値を使用します。",
...
def main() -> None:
  ...
  single_parser.add_argument(
    "--output", type=Path, default=None, help="分析結果を保存するmdファイルパス"
  )
  single_parser.add_argument(
    "--dry-run",
    action="store_true",
  ...
  batch_parser.add_argument(
    "--output-dir",
    type=Path,
    default=Path("reports/llm_batch"),
  ...
  if args.command == "single":
    ...
    if args.daily_check_report is None and args.tickers is None:
      raise SystemExit(
        "--daily-check-report か --tickers のどちらかを指定してください。"
      )
