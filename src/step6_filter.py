...
def apply_final_filter(df: pd.DataFrame,
                       dd_pred_proba: pd.Series = None,
                       dd_threshold: float = 0.60,
                       atr_percentile_limit: float = 0.90,
                       zscore_lower_limit: float = -2.5,
    ...
    # ドローダウン許容判定
    if use_ml and dd_pred_proba is not None:
        ...
        d["drawdown_prob"] = np.nan
        # MLを使わない場合: entry_score >= 閾値 を安全判定とする
        safe_drawdown = d["entry_score"] >= score_threshold
    ...
    # Final entry decision with safety filters and high_atr_bottoming exception
    safe_condition = safe_drawdown & safe_zscore & (safe_atr | (d["signal_type"] == "high_atr_bottoming"))
    d["final_decision"] = np.where(
        (d["signal_type"] != "overheat") &
        ((d["entry_score"] >= 4) | ((d["signal_type"] == "pullback") & (d["entry_score"] >= 3))) &
        safe_condition,
        1.0, 0.0)

    # ML使用時かつ検証期間外の行は判定も NaN とする
    if use_ml and dd_pred_proba is not None:
    ...
    d["filtered_by_atr"]      = np.where(is_bullish & (~safe_atr),      1, 0)
    d["filtered_by_zscore"]   = np.where(is_bullish & (~safe_zscore),   1, 0)
...
def evaluate_final_performance(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    filtered_atr    = valid_df["filtered_by_atr"].sum()    if "filtered_by_atr"    in valid_df.columns else 0
    filtered_zscore = valid_df["filtered_by_zscore"].sum() if "filtered_by_zscore" in valid_df.columns else 0

    print("\n" + "=" * 50)
    print("ステップ6 意思決定の適用サマリ (検証対象データのみ)")
    print("=" * 50)
    print(f"元の強気シグナル件数  : {total_bullish:>5} 件")
    print(f"最終エントリー件数    : {final_entries:>5} 件")
    print(f"見送り件数（合計）    : {total_bullish - final_entries:>5} 件")
    print("-" * 50)
    print(f"  うち ドローダウン予測/スコアフィルタ : {filtered_dd:>4} 件")
    print(f"  うち ATRボラティリティフィルタ       : {filtered_atr:>4} 件  [atr_percentile >= 0.90]")
    print(f"  うち zscore底抜けフィルタ            : {filtered_zscore:>4} 件  [dev_ma25_zscore <= -2.5]")
    ...
    if "tb_label" in valid_df.columns and "tb_return" in valid_df.columns:
        ...
        if len(bullish_df) > 0:
            ...
            raw_pf = raw_profit / raw_loss if raw_loss > 0 else (np.inf if raw_profit > 0 else 1.0)
        ...
        if len(final_df) > 0:
            ...
            final_pf = profit / loss if loss > 0 else (np.inf if profit > 0 else 1.0)
            print(f"\n[フィルタ後] 最終エントリー ({len(final_df)} 件)")
            print(f"  勝率: {final_win_rate:.1%}  |  平均リターン: {final_ret:.2%}  |  PF: {final_pf:.2f}")
        ...
        perf_records.append({
            ...
            "mean_return": final_ret,
            "profit_factor": final_pf
        })
        
    return pd.DataFrame(perf_records).set_index("stage")
