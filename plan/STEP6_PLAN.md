...
指摘された4つの懸念事項を踏まえ、以下の改善プランをステップ6に適用します。

### 5-1. OOS外データの適切な除外（優先度：高）
*   **課題**: 時系列検証（Walk-Forward Validation）の対象外となる初期期間で `drawdown_prob` が `0.0` に初期化され、リスクが低いと誤判定されてすり抜けている。
*   **対策**: `dd_pred_proba` で確率が得られない（OOS外の）行は `NaN` とし、フィルタ適用パフォーマンスの評価対象から除外（ドロップ）する。

### 5-2. ドローダウン閾値の最適化（優先度：最優先）
*   **課題**: 現在の閾値 `0.40` は厳しすぎて、勝率78%を超えるような良好な「買い」取引まで「ドローダウン予測」で却下してしまい、フィルタが逆機能している。
*   **対策**: 閾値（`drawdown-prob-limit`）を `0.60` 〜 `0.70` の範囲に引き上げ、本当にリスクが高い（ドローダウンの発生確率が極めて高い）タイミングのみを除外するように条件を調整する。

### 5-3. `dev_ma25_zscore` による極端な逆張りフィルタの実装（優先度：中）
*   **課題**: 設計プランに盛り込まれていた `dev_ma25_zscore <= -2.5`（底抜け逆張りリスクの排除）が未実装。
*   **対策**: `filtered_by_volatility` に `d["dev_ma25_zscore"] <= -2.5` 条件を追加する。

### 5-4. 順張りシグナルフラグ（`trending_bullish_flag`）の活用（優先度：低）
*   **課題**: フェーズ3で実装した順張りフラグが最終エントリーの絞り込みに考慮されていない。
*   **対策**: 順張りシグナルの日（`trending_bullish_flag == 1`）はトレンドが強くドローダウンを一時的に耐えられる可能性が高いため、ドローダウン閾値を緩和（例: `dd_threshold = 0.65`）し、逆に逆張り型強気の日（フラグが0）は厳しくチェックする動的閾値設計を検討する。
