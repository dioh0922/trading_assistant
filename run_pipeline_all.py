...
def run_step_llm(ticker: str | None = None, dry_run: bool = False) -> bool:
  ...
  if ticker:
    ...
    if daily_check_report is None:
      log.error(
        "日次レポート (daily_check*.md) が見つかりません。dailyステップを先に実行してください。"
      )
      return False
...
def main():
  ...
  parser.add_argument(
    "--ticker", type=str, default=None, help="特定の銘柄コードのみ処理する場合に指定"
  )
  parser.add_argument(
    "--force", action="store_true", help="モデル学習を強制再実行する (--force)"
  )
  parser.add_argument(
    "--dry-run",
    action="store_true",
    help="LLM分析でAPI呼び出しを行わずプロンプトを表示のみ",
  )
